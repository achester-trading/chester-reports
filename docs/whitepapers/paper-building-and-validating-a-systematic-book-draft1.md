...
**`ib_insync` is the async Python wrapper**, and the one the roadmap names. One caution for build time: the original `ib_insync` project was archived after its author's death in 2024 and the maintained continuation is `ib_async`. Verify which is current before Gate 1; the API surface is the same.
