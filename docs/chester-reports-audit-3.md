...
| cron-job.org | **Deleted by the operator on 30 September 2026** | Correct sequencing (prove, then delete) — and the sequence changed target: the GitHub `schedule:` block that was to replace it is gone too, because the store, the object and the register live on the box and a runner would print five absences. `chester-monthly.timer` on the 1st at 07:30 ET is the scheduler; `workflow_dispatch` stays as the manual fallback |
...
| **METHOD VERSIONS of the market-state object** | A bump means the CODE that builds the object changed on purpose, as distinct from config_version (the rules changed) and from a regression. Named here because a bump nobody recorded is a bump nobody can date, and `tools/validate_regime.py` fails if `regime.py` or `contradictions.py` changes without one. | — | `market-state-method-1` (20 Sep 2026) -- the first, covering the object as built through Phase 2b: eight dimensions on declared bands, three dials, the seven-row contradiction table, persistence, and exceptions[]. **`market-state-method-2`** (20 Sep 2026) -- session event classes on the object (OPEX, TRIPLE_WITCHING, MONTH_END, QUARTER_END, INDEX_REBALANCE, ETF_REBALANCE), and the gamma dial marked PROVISIONAL with low confidence on an expiry session, because a profile measured over open interest that settles that morning describes a book about to stop existing. **`market-state-method-3`** (20 Sep 2026) -- the vol dial's term-structure leg, computed from `calc.vix3m_over_vix` as a DECLARED PROXY for the VX futures curve, with its own bands and the same two-session persistence rule the dimensions use. The CFE VX1-VX3 keys stay declared as the champion the proxy will be measured against for a full quarter before either is retired. **`market-state-method-4`** (23 Sep 2026) -- the vol dial's realized/implied leg computes instead of reporting a permanent absence. Its ratio bands were declared in config v1.1 and never applied: the leg checked that `calc.vol_spy_realized_20d` had rows and then returned state None regardless, while the `implied_vs_realized_vol` contradiction row read that same series the whole time. One concept with two consumers, one of them blind. Both now read the same realized key and the same implied key -- the dial's own primary, so the ratio is same-day on both legs -- and neither computes a standard deviation of its own. **`market-state-method-5`** (23 Sep 2026) -- the term-structure leg runs a DUAL. Cboe's public per-contract settlement files turned out to be reachable without a key, so `calc.vx_front_ratio` (VX2/VX1 from real settlements) computes as CHAMPION beside `calc.vix3m_over_vix` as challenger, each with its own bands, its own persistence run and its own history; the object records both states, which of them was published, whether they agreed, and `calc.vx1_vix_basis` -- the futures-to-index dislocation the proxy structurally cannot see, because both of its legs ARE the index. The challenger stays published until 23 Dec 2026: reports have been read against its state and switching silently would change what a printed word means. The two do not agree about magnitude even when they agree about direction -- on 18 Sep the proxy read 1.2316 at its 94th percentile while the curve read 1.0485 at its 37th, so 'unusually steep' and 'ordinary' were the same session. That disagreement is what the quarter is for. **`market-state-method-6`** (23 Sep 2026) -- THE DIMENSIONS READ DERIVED MACRO SERIES. Sahm, the year-over-year family, 2s10s, r-vs-g and net liquidity were `monthly_macro/compute.py`: one report's private arithmetic over the CSV store, no history, no percentile, unreadable by the object that has a macro dial to fill. They are fifteen `calc.*` metrics now, and three dimensions read them -- liquidity's PRIMARY is `calc.net_liquidity` (the quantity, where the primary had been `fred.rrp`, one drain standing in for it because it was the only daily leg), rates supports on `calc.yield_curve_2s10s` and growth on `calc.sahm_rule`, both at polarity -1 and neither able to move a state alone. Liquidity's cadence moved daily to weekly with its primary, which is the honest cadence of a quantity whose slowest leg prints on Wednesdays. The code in `regime.py` did not change; config v1.8 did, and the version is bumped anyway because an object whose liquidity state came from one drain is not comparable with one whose state came from the whole. **The port found three defects**: TGA was declared in billions and is millions, so `fed_net_liquidity` returned -823.6 trillion for its entire life and the Monthly printed it; the Sahm reading took the low of raw monthly levels instead of the low of the 3-month averages, biasing it toward its own 0.50 trigger (0.333 against 0.20 on April 2026); and r-vs-g paired the latest yield with the latest inflation print across five weeks of vintage. |
| **4b — The Monthly, halved — 23 Sep 2026** | eleven pillars subordinated to three dials with a declared mapping (`config/pillars.yaml`) · six sections of change in place of ten pillar pages · the regime READ from the object, and the ten "characterise the regime from the pillars" placeholders deleted rather than repointed · every scenario weight beside its Brier · Top & Bottom and Alternative Assets as sections · the old pillar pages compressed to one delta row per series in the appendix · runtime moved from GitHub Actions to `chester-monthly.timer` on the box | 6 | **The regime is stated once, by `regime.py`, and the Monthly reads it.** The report no longer asks prose to supply a regime ten times over with no obligation to agree, and each pillar prints beneath the dial it feeds with its weight. The timer ships DISABLED: the operator enables it, and the heartbeat watches it only once enabled. |
| **2b — Feed the object — 20 Sep 2026** | prices on a schedule (28 symbols, 16:10 and 06:45) · the FRED history moved to the box and pulled there daily · breadth, trend and realized vol · the exceptions branch | 4 | The object's inputs arrive on the box without a human. **NOTE, now discharged (Phase 4b piece 4, 23 Sep 2026):** the Monthly kept its OWN FRED and yfinance pull while GitHub Actions ran it, because a runner starts with an empty store and has to fill one. On the box it does not: `scripts/run_monthly.sh` passes `--skip-fetch`, so the 06:45 overnight pass is the one owner of the pull and the Monthly renders what that pass left. `CHESTER_MONTHLY_FETCH=1` restores the old behaviour for a checkout with no feeds behind it. The duplication was idempotent and cheap; removing it is not about the fetch but about who is accountable for the data being there — with two pullers, a stale store could be either one, and the Monthly patching over a broken overnight pass is how a feed outage stays invisible. |
| **2 — State — DONE 19 Sep 2026** | regime.py → the market-state object v1 with 8 dimensions · standard derived forms · contradiction table v1 | 8 | **Met.** Every report reads the object (`regime.latest()`; the close pass is the only writer) and both anchors open on a WHAT CHANGED block. Caveat recorded rather than waived: 3 of the 8 dimensions and 5 of the 6 contradiction pairs are ABSENT for want of data — nothing schedules `yfinance_source.py`, so the store holds no equity prices, and the FRED series stop at 28 May. See `market-state.md` §5. |
...
| cron-job.org | Part 24 | Replaced first by a `schedule:` block, then by the box: `chester-monthly.timer`, 1st at 07:30 ET, shipped disabled and enabled by the operator. **Deleted by the operator on 30 September 2026** — and the `schedule:` block went with it, because a GitHub runner has no store, no object and no register, so a scheduled CI Monthly would build a report of absences and mail it | Architecture Part 32.1; Phase 4b piece 4 | Never |
