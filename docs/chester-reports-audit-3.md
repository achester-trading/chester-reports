...
| **2b — Feed the object — 20 Sep 2026** | prices on a schedule (28 symbols, 16:10 and 06:45) · the FRED history moved to the box and pulled there daily · breadth, trend and realized vol · the exceptions branch | 4 | The object's inputs arrive on the box without a human. **NOTE, carried deliberately:** the Monthly keeps its OWN FRED pull (`monthly_macro/run.py` calls `yfinance_source.pull` and the FRED source directly) until Phase 4 relocates that report to the box. Two pulls of the same series is idempotent in the observation store, so the duplication costs a fetch and no rows -- but it is duplication, and Phase 4 is where it goes. |
