...
| ST-4 | Features — Duration Absorption and rates flags. First: G-30 (auction timestamp from the API, 13:05 ET fallback) and G-31 (rename the SEP/FedWatch keys). Then `calc.net_supply_*` (G-14 mapping: MSPD total marketable held by the public, month-on-month, minus Δ `fred.soma_treasuries`), `calc.priv_residual_12m`; `auction.*` features and `ABS_STRESS` on cover and dealer share (G-32); TIC composition and holder shares, `ABS_FRAGILITY` (display only); `calc.fed_mkt_gap_*`, `CRED_STRESS`, `calc.pm_disagree_fed` (FedWatch by hand until G-4); `DEANCHOR`, `calc.be5y5y_range_pos`, `calc.producers_spot_ratio`; `calc.self_fund_ratio`, `calc.hs_netdebt_12m`, `calc.aa_bbb_oas_diff`, `calc.ai_long_share` (from the G-35 manual keys), `calc.mu_inv_days`; `calc.cb_gold_12m` (from the IMF IFS module added here per G-34), `calc.tic_official_12m`, `calc.tic_private_12m` | 6.5 | ST-1, ST-2, ST-3 |
...
| G-20 | **Closed 26 Sep (ST-1).** The daily FRED pull runs inside `altdata/feeds.py` (`pull --skip loggers`, called by `scripts/run_eod_cron.sh` before the 16:45 object); `FRED_PULL_SERIES = FRED_SERIES + FRED_SIGNAL_SERIES`, so the 17 new series ride the existing pull without touching the Monthly's 59; the five ST-1 sources are registered as `OFFICIAL_WRITERS` on the same pass. TODO.md's "schedule a FRED pull" line is stale. | — | — |
...
| G-30 | Auction `available_at`: ST-1 stamped a constant 13:00 ET; TreasuryDirect stamped the 24 Sep 7-year result at 13:03:20. Ruling: use the API's own result timestamp when present, 13:05 ET as the fallback for rows without one (`AVAILABLE_ET` in `treasury_auctions.py`). | ST-4 (first item) | any auction feature |
| G-31 | SEP / FedWatch manual keys: ST-1 defined `sep_median_next2` as the current year-end dot and `sep_median_12m` as the year-end dot nearest 12 months out. Ruling: rename while empty to `manual.sep_median_ye_current`, `manual.sep_median_ye_next`, `manual.fed_signalled_direction` (hike / hold / cut from the last statement), `manual.fedwatch_next_hike_prob`, `manual.fedwatch_next_cut_prob`; the gap metric compares the market's implied move at the next meeting with the signalled direction, and the 12m leg with the nearest year-end dot. | ST-4 | `calc.fed_mkt_gap_*` |
| G-32 | No free source for the when-issued yield, so `auction.tail_bp` is registered `not_yet_sourced` and never written. Ruling: until sourced, `ABS_STRESS` = bid-to-cover in its bottom decile on two consecutive coupon auctions, or dealer takedown > 20%; ST-7 tests those two, plus a pseudo-tail (high yield minus the prior close's constant-maturity yield) as a candidate only, on the 6,041-auction history. SR-22's flag definition reads accordingly until a tail source exists. | ST-4, ST-7 | `ABS_STRESS` |
| G-33 | ACM reads STALE on the box until `xlrd` is installed in its venv (`pip install -r requirements.txt` there); the deploy does not install packages. | operator, after the ST-1 push | first ACM row on the box |
| G-34 | **Ruled 26 Sep (after ST-2).** World Gold Council files are login-gated; `wgc.*` stays declared and never written. IMF International Financial Statistics (official gold holdings, tonnes, by country, monthly, free API) is the source for SR-20's monthly gold proxy — `imf.ifs_gold_t` by country and total, `calc.cb_gold_12m` computed from it; WGC remains a by-hand cross-check at the annual scorecard. | ST-4 (feed added there, one module) | `calc.cb_gold_12m` |
| G-35 | **Ruled 26 Sep (after ST-2).** SIFMA issuance-by-maturity tables are not freely fetchable; `sifma.*` is NOT AVAILABLE. Two quarterly `manual_input` keys replace it — `manual.ig_issuance_10y_plus_share` and `manual.ai_ig_issuance_share` (the Goldman-class figures SR-16 cited) — and `calc.ai_long_share` reads them until a free source appears. | ST-4 | `calc.ai_long_share` |
