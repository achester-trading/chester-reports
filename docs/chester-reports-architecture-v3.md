...
The operator's binding constraint is time, and a report that must be opened is a report that will be read late or not at all. **Delivery is by HTML email rendered in the message body**, sent through an authenticated SMTP relay on the operator's own mail account rather than direct from the box, because direct sending from a cloud host lands in spam. A copy is written to `reports/` in the checkout FIRST, before the socket is opened, so the record does not depend on an inbox and a mail server having a bad day costs the delivery and never the report. **Amended 6 Sep 2026: `reports/daily_close_*.html` is gitignored, not committed.** The archive lives on the box and is swept up by the off-box backup (D1a); committing one HTML file per session forever, from a box whose standing rule is that it runs code and never edits it (Part 25), is a different decision than keeping a durable record, and the backup already satisfies the requirement this line exists for. Revisit only if a report needs to be diffable in git history.
