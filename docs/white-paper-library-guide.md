...
> **Formats — the durable rule.** For the guide and for every paper that carries computed figures — Currencies (V), the Dealer's Hand (XIII), Technical Indicators (XIV), Options as Expression (XXII), Evidence and Inference (XXIII), Earnings (XXIV) — **the HTML edition is canonical for reading and the Markdown edition is canonical for editing.** The figures live only in the HTML; the `.md` carries placeholders. The built editions live in `docs/html/`, one per paper, named `<slug>-whitepaper.html`. The durable copy of this rule lives in `CLAUDE.md`, which no upload flow touches, and CLAUDE.md wins if the two disagree. **If this note is absent from the guide, that is evidence of a stale full-file re-upload, not of the rule being retired** — restore from CLAUDE.md, and re-upload the guide only from a pulled copy or by editing the `.md` in the repository.
...
|  | **V** | Currencies: A Structural Framework | Currencies | Foundational to every dollar-priced asset | v1.0 · ~15,400 words, 8 FX figures · HTML canonical | How exchange rates move and who moves them, the dollar question, digital assets as currency, hedging and trading FX risk |
