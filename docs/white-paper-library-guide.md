...
> **Formats — the durable rule.** **The Markdown is canonical for everything** — this guide and every paper. A paper's `.md` under `docs/whitepapers/` holds its text, its tables and its figure references, and it is the only file anyone edits. **Figures are files** under `docs/figures/<slug>/`, named `fig-NN.svg`, with an `index.md` per paper mapping number → caption → file; a paper references one with an ordinary image link. **`docs/html/` is build output** — one `<slug>-whitepaper.html` per roster paper, written by `tools/build_paper_html.py` with the figures embedded, rebuilt by `make html`. Never edit a built edition and never upload one into `docs/html/`. The earlier rule — *the HTML edition is canonical for reading* — is **retired**: it existed only because six papers' figures were trapped inside their HTML, and they are not any more. What survives is the upload discipline that rule kept failing to teach: **a full-file re-upload from a stale local copy silently reverts whatever the repo learned since** — it destroyed this very note twice in one day — so pull first, or edit the `.md` in the repository. The durable copy of all of this lives in `CLAUDE.md`, which no upload flow touches, and CLAUDE.md wins if the two disagree.
...
|---|---|---|---|---|---|---|
| Foundational | **XXV** | Price, Time, and Edge | Price, Time, and Edge | Every markets paper — it supplies the vocabulary they cite; read second in the core track | v1.1 · ~29,500 words | The layer the library was missing — **a price is the expectation of the marginal participant, weighted by his capital and his urgency**, markets trade on the difference between reality and expectations, and price is itself a participant; the seven-rung horizon ladder and how the rungs couple, with **the weeks-to-months rung given its own craft — the five forces that move price over weeks, why the middle rung is the emptiest for a discretionary individual, entry and exit as the rung defines them, and the mechanism behind the two conversions that kill a swing book**; the participants at full depth — holders, intermediaries, speculators, issuers and sovereigns by liability, mandate, regulation and calendar, the seven forcing mechanisms, and "who is on the other side" as a method with worked identifications; and the theory of edge — six limits to arbitrage, **the twelve edges with capacity and death conditions**, the capacity arithmetic that says a small book's edges are the capacity-constrained ones and no others, and the six questions with eight worked theses, two of which pass and six of which fail one question each |
...
|  | **IV** | The Rate and Liquidity Machine | Rates & Liquidity | Monthly Macro (liquidity, sovereign) | v1.0 · ~16,450 words | Eleven regime eras 1907–Warsh, ~fifty policy and plumbing instruments across six families, the 1942–51 yield peg as the live analogue |
|  | **V** | Currencies: A Structural Framework | Currencies | Foundational to every dollar-priced asset | v1.0 · ~15,400 words, 8 FX figures in `docs/figures/` | How exchange rates move and who moves them, the dollar question, digital assets as currency, hedging and trading FX risk |
| Structural asset classes | **VI** | Credit: A Structural Framework | Credit | Top & Bottom, Monthly (credit pillar), Disruptive Themes | v1.0 · ~18,100 words | A lending-standards cycle, not a spread cycle — private credit and bifurcation, credit as a timing tool, the seven concealment mechanisms, plus the library's trading-the-curve section |
|  | **VII** | Energy: A Structural Framework | Energy | Alternative Asset (oil, gas), Monthly (inflation, geopolitics) | v1.0 · ~12,900 words | A buffer market, not a resource market — and electricity as the scarce commodity; oil, gas, power, uranium, the Hormuz test |
|  | **VIII** | Metals: A Structural Framework | Metals | Alternative Asset | v1.0 · ~17,100 words | Gold, silver, copper as three distinct assets — stock-vs-flow frame, signal hierarchies, false signals, position construction |
|  | **IX** | Digital Assets: A Structural Framework | Digital Assets | Alternative Asset | v1.0 · ~18,900 words | Long-horizon crypto self-sufficiency — taxonomy, value accrual, honest bull and bear, forecast track records |
| Market timing | **X** | Tops and Bottoms: Every Major U.S. Turning Point Since 1970 | Tops and Bottoms | Top & Bottom | v1.0 · ~10,550 words · 3 formats from one source | Eight bears and the near-misses on one template, forward returns standardized, calibration against 13 episodes |
|  | **XX** | Base Rates: What Markets Usually Do | Base Rates | Every thesis, before it is written; Top & Bottom (extended episode set); the tail watch | Draft 1.3 · ~10,000 words | **An executive summary of twelve takeaways up front, and a 'what to take from this table' block after every table in Parts I–II, each tied to a Doctrine rule or a known bias;** the denominators — return and drawdown distributions, streaks and mean reversion, correlation by regime, earnings and options base rates, costs; seasonality with its decay evidence and the midterm-cycle conditional; percentiles (p25/median/p75) throughout rather than means; the pre-1970 downturn extension (1907, 1929–32, 1937–38, 1946–47, 1961–62) that takes the Tops & Bottoms calibration sample from eight bears to thirteen (the NARRATED roster; the computed table counts twelve from the running maximum, per that paper’s erratum of 23 September 2026); **and Part IV — how bad it can get outside the U.S. record: Japan's 34 years, Weimar, Greece, Argentina, the markets that went to zero, the long-term debt cycle's two resolutions, and what protected capital in each** |
| Micro & execution | **XI** | Volatility: A Structural Framework | Volatility | Execution layer, Daily Cascade, expressibility | v1.0 · ~14,950 words | The price of insurance — VRP, crowding and unwinds as one fact; regime identification and cross-asset vol; the options-ETF complex at scale |
|  | **XII** | The Daily Cascade Paper | Daily Cascade paper | Daily Cascade | **v2.0 · ~21,450 words** | Field guide to the 24 analytical sections (seven-part treatment each), the six-cluster dependency map and the three-of-four-cluster confluence rule — retained from v1 — with **Part 0, the v2 architecture: the nine-slot cadence (three anchors, five conditionals, one alert) with a v1→v2 slot map, the two-anchor guarantee, sections mapped to slots and sources, no-slot-computes, the narrative register and the 07:00 scan, drafted setups into the register, payload → prose → numeral audit, delivery and grading; the temporal chain and failure modes rewritten; the backlog reconciled to Audit #3** |
|  | **XIII** | The Dealer's Hand | The Dealer's Hand | Daily Cascade (dealer positioning) | v1.1 · ~19,350 words, 21 computed figures in `docs/figures/` | Derives the dealer-hedging mechanics from first principles — sign conventions, who is on which side, the JHEQX exception — with 21 computed figures and 2 worked ledgers; **Part V adds the multi-horizon book (GEX/DEX by bucket, 0DTE session taxonomy)**; elaborates Daily Cascade Ch. 2 |
|  | **XIV** | Technical Indicators | Technical Indicators | Daily Cascade + execution layer | v1.1 · ~6,250 words, 5 figures in `docs/figures/` | Price-derived measures — the lag argument, the five permitted clusters, levels by mechanism, the traps, and the wall keeping technicals out of the composite |
|  | **XXII** | Options as Expression | Options as Expression | Book C's every trade; Book A's convexity sleeve; the expression check | Draft 1.4 · ~15,400 words, 37 figure panels in `docs/figures/` + a recomputable scenario matrix | Choosing the instrument after choosing the view — the four primitives as shapes, your greeks versus the dealer's, verticals and the four numbers, time spreads, **twelve volatility and convexity structures with a figure and a home-book each — including the collar and put-spread collar for Book A, the backspreads for the tail budget, and the reverse-iron-butterfly naming settled; the iron condor's arithmetic (risk seven to make three) and its four management rules; reverse iron condor, double diagonal, the LEAP diagonal; entering, margining and exiting multi-leg positions; and the excluded-structures table**; **Chapter 15, the scenario matrix — ten structures against the index across ten outcomes on four asset classes (U.S. equity, China, real estate, crypto) at twelve and six months, with the price-of-insurance table and seven decision rules for the monthly session**; **Chapter 16, leverage — the forms and their financing cost (futures, deep-ITM LEAPS, spreads, backspreads, cash-secured puts, short box spreads, leveraged ETFs and their variance tax), the volatility problem after a decline, five ways to add at the trough priced at 40% vol, the bottom-signal playbook as a sequence, and the exceptional-tier expression**; the three costs (decay, the volatility crush, the spread tax); and the engineered payoffs: buffered funds, **synthetic principal protection from a cash-equivalent fund plus long-dated calls**, and **dual-directional structures built as bills-plus-iron-butterfly rather than as barrier notes** — with the tenor arithmetic showing why option cost grows as √T while funding grows as T |
| Market structure | **XV** | Equities: A Structural Framework | Equities | Top & Bottom, Daily Cascade, Disruptive Themes (Factor II) | v1.0 · ~8,350 words | The residual claim, the index as a momentum machine, factors, the concentration and its fulcrum, the AI rings and the new listings, scenario families, what to watch by cadence |
|  | **XXI** | International Equities: Europe, Japan, China, and the Rest of the World | International Equities | Book A's allocation sleeve; Book B candidates; the Daily Cascade's overnight read; Disruptive Themes (AI supply chain); the tail watch (Taiwan) | Draft 1.1 · ~9,250 words | The global map and home-bias arithmetic; **the 55-year record of alternating leadership (five flips, ~decade runs, the current U.S. run the longest and widest)**; the decomposition of fifteen years of U.S. outperformance (a third currency, a third multiple, a third earnings) and which parts can repeat; the currency term and the hedged/unhedged decision as a recorded choice; Europe, UK, Japan, China, Taiwan/Korea, India and EM one at a time; instruments and their traps — stale NAV as live price discovery, withholding tax by account type, the session clock behind the overnight gap; **correlation's long rise, its regime dependence and downside asymmetry, and why daily correlation is a clock artifact**; **a full part on China — three markets, the dilution/state-allocation explanation of the lost decade, the technology record sector by sector, the winning-the-industry-without-earning-a-return trap, and four ten-year scenarios**; **and a ten-year probabilistic view of relative returns built from decomposed components, with falsification conditions per scenario** |
|  | **XXIV** | Earnings: The Reaction Is the Object | Earnings | The event-reaction setup; the cohort monitor's revision flags; the single-name books' valuation vocabulary | Draft 1 · ~4,250 words, 9 computed figures in `docs/figures/` | The print is not the event — the managed consensus, the whisper, the revision cycle and its second-derivative flag; the implied move against a name's own history, the event premium, positioning into the print; the four quadrants (guidance beats the print), gap–fade–drift, reaction-versus-expectation as the tradeable object; **the denominator problem — five P/Es on one stock**, when book value means anything, EV multiples, cash versus earnings, **the sector multiple map and the cycle inversion (cyclicals cheapest on trailing at the peak)**; sector grammar with insurance as the worked example |
| Positioning & the book | **XVI** | Positioning & Flows | Positioning & Flows | Every report's positioning line; the tail watch; the heat factor map | Draft 1.2 · ~11,700 words, 1 computed diagram | Who is forced to move, when, and whether the market is absorbing them — **the participant map — ownership by sector (sums to 100), cross-cutting mandates, trading behavior, and a WHO × HOW cross-tabulation with an owner→mandate→forcing-mechanism diagram**, ten mechanical-holder types plus the discretionary holders' mechanical failure modes (degrossing, redemptions, liability-driven rate shocks, currency-hedge rebalancing, market-maker withdrawal), the master calendar, absorption as response-per-unit-stimulus, six flow models with stated uncertainty, the seven-group independence table |
|  | **XVII** | Portfolio Construction Across Regimes | Portfolio Construction | All reports → positioning | Draft 1 · ~4,500 words | Preview edition — frame, vocabulary, sizing disciplines, open questions — with a four-condition trigger for the empirical Draft 2 (register data + IBKR Gate 1) |
...
**Companion to: every markets paper — it supplies the vocabulary they cite rather than restate · v1.1 · ~29,500 words, twenty-six chapters across five parts + two appendices, no figures**
...
calendar; the seven forcing mechanisms argued one by one, with March 2020, February
2018 and a short squeeze as worked instances; and "who is on the other side" as
a four-step method with six worked identifications — one from each group of
participants, and one that fails.
...
which is the paper's central practical claim; the six questions with eight
worked theses — two that pass, and one that fails at each of the six, so that
every question carries a failure the reader can recognize in his own packet;
several partially independent edges, opening with the four books taken from the
Doctrine's Part IV and the nine alpha families mapped onto the twelve; the small book stated as the market
sees it, with its shadow and with the standing argument for which edge families
a book of this size can actually reach; and how edges die, with a tripwire for
each. Part V
...
**Companion to: Alternative Asset Report — and foundational to every paper above it · ~14,800 words, 33 sections across eight parts · 8 FX figures in `docs/figures/currencies/`**
...
**Companion to: Daily Cascade — dealer positioning · word count not yet measured · 21 computed figures in `docs/figures/dealers-hand/` + 2 worked ledgers**
...
**Companion to: Daily Cascade and the execution layer · ~6,350 words, 8 chapters + reference appendix · v1.1 adds four numeric reading tables · 5 figures in `docs/figures/technical-indicators/`**
...
**Companion to: every thesis, before it is written; Top & Bottom (the extended episode set); the tail watch · Draft 1.2 · ~10,000 words, sixteen chapters across four parts + two appendices**
...
**Companion to: Book C's every trade; Book A's convexity sleeve; the expression check · Draft 1.4 · ~15,400 words, nineteen chapters across five parts + two appendices, 37 figure panels in `docs/figures/options-expression/`**
...
**Companion to: how every register figure is read; the promotion gates; the trust matrix's sample-size rules · Draft 1 · ~4,050 words, sixteen chapters across four parts + appendix, 5 figure placeholders — none drawn**
...
## XXIV. Earnings: The Reaction Is the Object
**Companion to: the event-reaction setup; the cohort monitor's revision flags; the single-name books' valuation vocabulary · Draft 1 · ~4,250 words, twenty chapters across six parts + appendix, 9 computed figures in `docs/figures/earnings/`**
...
seven categorical fields it will need, populated from day one. Real estate and
