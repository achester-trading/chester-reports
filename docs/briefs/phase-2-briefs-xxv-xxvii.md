...
- **Presentation standard** (adopted with XXV Draft 1.1, 7 September 2026; applies to every paper from here on):
  - *Confidence.* No `(mechanism)` marker anywhere — unmarked text is mechanism by default. An empirical claim keeps its sample period and its last-verified date, but as a short italic closing clause inside the sentence rather than a bracketed label, and the word "regularity" is retired as a tier name. Forecasts remain appendix-only.
  - *Prose before tables.* Any taxonomy, list or matrix is argued in paragraphs first and then summarized in a concise table. The table is a cheat sheet — fewer words than the prose it follows, for looking a thing up — and never the primary carrier of the argument. A claim a reader can find only in a table is a defect.
  - *Conceptual figures.* Rounded, order-of-magnitude figures for the participant classes' ownership and turnover shares are permitted in the timeless body when the table is labelled "conceptual; rounded; dated figures in Appendix B" and carries one sentence on why the methodology is imperfect. The gate continues to police exact figures with no as-of date, and no other level, probability or "today" read belongs in a timeless body.
